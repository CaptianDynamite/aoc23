module day13helper
    implicit none
contains
    impure function row_reflect_loc(map) result(loc)
        character, intent(in) :: map(:, :)
        integer :: loc
        integer :: fwd, back, i, m, mi
        ...
        do i = 1, size(map, 2) - 1
            ...
            inner: do while (i + back > 0 .and. i + fwd <= size(map, 2))
                if (any(map(:, i + back) /= map(:, i + fwd))) then
                    reflect = .false.
                    exit inner
                end if
                fwd = fwd + 1
...
program day13
    ...
    do while (.true.)
        ...
        if (iostat /= 0 .or. cur_line == '') then
            ...
            temp = row_reflect_loc(map)
            part1 = part1 + 100 * temp
            allocate(tmap(lines, line_length))
            if (temp == 0) then
                tmap = transpose(map)
                part1 = part1 + row_reflect_loc(tmap)
            end if
